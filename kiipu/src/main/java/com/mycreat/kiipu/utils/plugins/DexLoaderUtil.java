...
public class DexLoaderUtil {
    ...
    public static Class<?> loadClass(ClassLoader cl, String clazzName) throws ClassNotFoundException {
        return cl.loadClass(clazzName);
    }
    public static void call(ClassLoader cl, Class<?> myClass) {
        call(cl, myClass,"com.example.MyClass","test1", null);
    }

    public static Object call(ClassLoader cl, Class<?> myClass, String clazzName,String method, Class<?>[] types, Object ...args ) {
        try {
            if(myClass == null) {
                myClass = loadClass(cl, clazzName);
            }
            Object instance = myClass.getConstructor().newInstance();
            if(types != null) {
               return myClass.getDeclaredMethod(method, types).invoke(instance, args);
            }else{
               return myClass.getDeclaredMethod(method).invoke(instance);
            }
    ...
    public static synchronized Boolean injectAboveEqualApiLevel14(
            String dexPath, String defaultDexOptPath, String nativeLibPath, String className) {
